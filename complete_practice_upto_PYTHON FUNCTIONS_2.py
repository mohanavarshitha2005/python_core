...
# l=[('mohana',30),('teju',23),('vasavi',15),('asha',19)]
...
# Create an application of a simple ATM system where a user attempts to withdraw or deposit money. The system should verify sufficient balance before withdrawal, perform the transaction, and display the remaining balance using multiple functions.

def atm(balance,withdraw=0,deposit=0):
    if withdraw <=balance:
        balance-=withdraw
    if deposit>0:
        balance+=deposit
    return balance
print(atm(30000,5000))
print(atm(30000,5000,10000))

atm=lambda balance,withdraw=0,deposit=0:(balance-withdraw if withdraw<=balance  else balance ) + deposit
print(atm(30000,5000))
print(atm(30000,5000,10000))
...
def atm(balance, withdraw_amt=0, deposit_amt=0):
...
withdraw = lambda balance, amount: balance - amount if amount <= balance else balance

deposit = lambda balance, amount: balance + amount

atm = lambda balance, withdraw_amt=0, deposit_amt=0: deposit(
    withdraw(balance, withdraw_amt),
    deposit_amt
)
